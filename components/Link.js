...
import { prefetch as doPrefetch } from 'react-storefront/serviceWorker'
...
/**
 * This is an extension from the reactStorefront Link component to support multilingual subpath links
...
const Link = ({ as, href, prefetch, prefetchURL, pageData, onClick, children, ...other }) => {
  ...
  if (!children || typeof children === 'string') {
    return (
      <NextLink href={`/${locale}${href}`} prefetch={false} as={as && `/${locale}${as}`} passHref>
    ...
    return (
      <RootRef rootRef={ref}>
        <NextLink href={`/${locale}${href}`} prefetch={false} as={as && `/${locale}${as}`} passHref={passHref}>
